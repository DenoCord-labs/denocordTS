...
export const Messages = {
  ...
  BITFIELD_INVALID: (bit: any) => `Invalid bitfield flag or number: ${bit}.`,
  ...
  SHARDING_SHARD_NOT_FOUND: (id: any) => `Shard ${id} could not be found.`,
  SHARDING_ALREADY_SPAWNED: (count: any) => `Already spawned ${count} shards.`,
  SHARDING_PROCESS_EXISTS: (id: any) =>
    `Shard ${id} already has an active process.`,
  SHARDING_WORKER_EXISTS: (id: any) =>
    `Shard ${id} already has an active worker.`,
  SHARDING_READY_TIMEOUT: (id: any) =>
    `Shard ${id}'s Client took too long to become ready.`,
  SHARDING_READY_DISCONNECTED: (id: any) =>
    `Shard ${id}'s Client disconnected before becoming ready.`,
  SHARDING_READY_DIED: (id: any) =>
    `Shard ${id}'s process exited before its Client became ready.`,
  SHARDING_NO_CHILD_EXISTS: (id: any) =>
    `Shard ${id} has no active process or worker.`,
  SHARDING_SHARD_MISCALCULATION: (shard: any, guild: any, count: any) =>
  ...
  INTERACTION_COLLECTOR_ERROR: (reason: any) =>
  ...
  VOICE_STATE_INVALID_TYPE: (name: any) => `${name} must be a boolean.`,

  REQ_RESOURCE_TYPE:
    "The resource must be a string, Buffer or a valid file stream.",

  IMAGE_FORMAT: (format: any) => `Invalid image format: ${format}`,
  IMAGE_SIZE: (size: any) => `Invalid image size: ${size}`,
  ...
  INVALID_TYPE: (name: any, expected: any, an = false) =>
    `Supplied ${name} is not a${an ? "n" : ""} ${expected}.`,
  INVALID_ELEMENT: (type: any, name: any, elem: any) =>
  ...
  THREAD_INVITABLE_TYPE: (type: any) => `Invitable cannot be edited on ${type}`,
  ...
  MISSING_MANAGE_EMOJIS_AND_STICKERS_PERMISSION: (guild: any) =>
  ...
  COMMAND_INTERACTION_OPTION_NOT_FOUND: (name: any) =>
    `Required option "${name}" not found.`,
  COMMAND_INTERACTION_OPTION_TYPE: (name: any, type: any, expected: any) =>
    `Option "${name}" is of type: ${type}; expected ${expected}.`,
  COMMAND_INTERACTION_OPTION_EMPTY: (name: any, type: any) =>
  ...
  MODAL_SUBMIT_INTERACTION_FIELD_NOT_FOUND: (customId: any) =>
  ...
  NOT_IMPLEMENTED: (what: any, name: any) =>
  ...
  InvalidIntents: "",
  DisallowedIntents: "",
  COMPONENTS_LENGTH_EXCEEDED: (length: number) =>
