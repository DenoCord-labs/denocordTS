...
export class ActionRow {
  ...
  create() {
    ...
    if (this.components.length > 1 && this.components[0] instanceof SelectMenu)
      throw new Error("You can't add more than one SelectMenu to an ActionRow");
    if (this.components.length > 5) {
      throw new Error("You can't add more than 5 components to an ActionRow");
    }
    return {
      type: 1,
